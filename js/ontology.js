	...
	$.get("rdf/historicmapsphen.rdf", function(xmlResponse){
		...
		for(var i = 0; i < rdfClasses.length; i++){
			if(rdfClasses[i].children.length == 0){
				$("#nav").append("<p id='pOntologyContentTag" + counter +"'><input type='checkbox' id='" + rdfClasses[i].name + "' value='" + rdfClasses[i].uri + "' class='chOntologyContent' >" + rdfClasses[i].name + " - <a href='" + rdfClasses[i].uri + "' target='_blank'>view</a> <a href='javascript: void(0)' onclick='removeElement(&quot;pOntologyContentTag" + counter + "&quot;)'>remove</a></p>");				
				counter++;
			}
		}
	}) 
